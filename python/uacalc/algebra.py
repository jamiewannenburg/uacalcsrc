...
try:
    from .uacalc_rust import rust_create_subalgebra
    HAS_RUST_SUBALGEBRA = True
except ImportError:
    HAS_RUST_SUBALGEBRA = False
...
def create_subalgebra(algebra: Algebra, generators: List[int], 
    ...
    This function uses an optimized Rust implementation that maintains
    parent algebra links and provides efficient index mapping between
    parent and subalgebra elements.
    
    Args:
        algebra: Original algebra
        generators: List of generator elements
        with_progress: Optional progress callback (accepted for API compatibility,
                      may be ignored in current implementation)
    ...
    for generator in generators:
        if not isinstance(generator, int) or generator < 0 or generator >= algebra.cardinality:
            raise ValueError(f"Invalid generator {generator}: must be integer in range [0, {algebra.cardinality})")
    
    # Generate default name if algebra doesn't have one
    algebra_name = getattr(algebra, 'name', 'algebra')
    name = f"{algebra_name}_sub"
    
    # Always try Rust implementation first
    if HAS_RUST_SUBALGEBRA:
        try:
            return rust_create_subalgebra(name, algebra, generators)
        except Exception as e:
            # Fall back to Python implementation if Rust fails
            warnings.warn(f"Rust subalgebra creation failed: {e}. Falling back to Python implementation.")
    
    # Fallback to Python implementation only if Rust is not available
    return _create_subalgebra_python_fallback(algebra, generators, with_progress)


def _create_subalgebra_python_fallback(algebra: Algebra, generators: List[int], 
                                     with_progress: Optional[Callable[[float, str], None]] = None) -> Algebra:
    """Fallback Python implementation for subalgebra creation.
    
    This is the original implementation that manually computes the closure
    and creates a new BasicAlgebra. Used when the Rust implementation is
