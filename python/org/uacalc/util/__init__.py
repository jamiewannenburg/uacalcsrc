...
if sys.platform.startswith("java"):
    ...
    IntArray = globals().get("IntArray")
    if IntArray is not None:
        # Methods already named get/set/clone-style on the binding; ensure
        # clone() exists for free_algebras.py (el.clone()).
        if not hasattr(IntArray, "clone") and hasattr(IntArray, "clone_array"):
            IntArray.clone = IntArray.clone_array
