    ...
    T: Clone + std::fmt::Debug + std::fmt::Display + std::hash::Hash + Eq + Send + Sync + 'static
{
    ...
    // Convert to i32 algebra for BigProductAlgebra
    let card = alg.cardinality();
    let ops = alg.operations();
    ...
    // Create BigProductAlgebra with power = arity
    let alg_boxed: Box<dyn SmallAlgebra<UniverseItem = i32>> = Box::new(i32_alg);
    let product_alg = BigProductAlgebra::new_power_safe(alg_boxed, arity)?;
    let product_alg_arc = Arc::new(product_alg);
    
    let size = card as i32;
    
    // Initialize arrays A, B, and G
    let mut a = vec![0i32; arity];
    let mut b = vec![0i32; arity];
    let mut g = vec![vec![0i32; arity]; arity];
    
    // Use nondecreasingSequenceIncrementor for A (values 0 to size-1)
    let mut a_inc = SequenceGenerator::nondecreasing_sequence_incrementor(&mut a, size - 1);
    
    loop {
        // Get current A values from incrementor
        let a_clone = a_inc.get_current();
        let a_array = IntArray::from_array(a_clone.clone())?;
        
        // Reset B to all zeros
        for i in 0..arity {
            b[i] = 0;
        }
        
        // Use sequenceIncrementor for B (values 0 to size-2)
        let mut b_inc = SequenceGenerator::sequence_incrementor(&mut b, size - 2);
        
        loop {
            // Get current B values from incrementor
            let b_clone = b_inc.get_current();
            
            // Construct generators G[j] for each j
            let mut gens = Vec::new();
            for j in 0..arity {
                for k in 0..arity {
                    if j == k {
                        // G[j][j] = B[j] + 1 if B[j] >= A[j], else B[j]
                        if b_clone[j] >= a_clone[j] {
                            g[j][k] = b_clone[j] + 1;
                        } else {
                            g[j][k] = b_clone[j];
                        }
                ...
                gens.push(IntArray::from_array(g[j].clone())?);
            }
            
            // Check if Aarray is in the subuniverse generated by the gens
            let mut closer = Closer::new_safe(product_alg_arc.clone(), gens)?;
            closer.set_element_to_find(Some(a_array.clone()));
            closer.set_suppress_output(true);
            
            let sub = closer.sg_close()?;
            
            if !sub.contains(&a_array) {
                // The tuple A is not in the subuniverse generated by the gens
                return Ok(false);
            }
    ...
    // All combinations passed, so there is an NU term
    Ok(true)
}
    ...
    T: Clone + std::fmt::Debug + std::fmt::Display + std::hash::Hash + Eq + Send + Sync + 'static,
{
    ...
    // Trivial algebra (cardinality 1) always satisfies SD-meet
    if n <= 1 {
        return Ok(None);
    }
...
/// a Day quadruple in the square of the free algebra on 2 generators.
/// Uses the faster method if the algebra is idempotent.
///
/// # Arguments
/// * `alg` - The algebra to check
///
...
where
    T: Clone + std::fmt::Debug + std::fmt::Display + std::hash::Hash + Eq + Send + Sync + 'static
{
    // If idempotent, use the faster method
    if alg.is_idempotent() {
        return is_congruence_modular_idempotent(alg);
    }
    ...
    // Create generators for F(2)^2:
    // a = (0, 0) - both coordinates are the 0th element of F(2) (first generator)
    // b = (0, 1) - first coordinate is 0th element, second is 1st element
    // c = (1, 0) - first coordinate is 1st element, second is 0th element
    // d = (1, 1) - both coordinates are the 1st element of F(2) (second generator)
    let a = IntArray::from_array(vec![0, 0])?;
    let b = IntArray::from_array(vec![0, 1])?;
    let c = IntArray::from_array(vec![1, 0])?;
    let d = IntArray::from_array(vec![1, 1])?;
    
    let gens = vec![a.clone(), b.clone(), c.clone(), d.clone()];
    ...
    // Create SubProductAlgebra generated by a, b, c, d
    let mut sub = crate::alg::SubProductAlgebra::new_safe(
        "F2SquaredSub".to_string(),
        f2_squared,
        gens,
        false, // find_terms not needed
    )?;
    
    sub.make_operation_tables();
    
    // Get element indices
    let a_index = sub.element_index(&a).ok_or_else(|| {
        "Element a = [0, 0] not found in subalgebra".to_string()
    })?;
    let b_index = sub.element_index(&b).ok_or_else(|| {
        "Element b = [0, 1] not found in subalgebra".to_string()
    })?;
    let c_index = sub.element_index(&c).ok_or_else(|| {
        "Element c = [1, 0] not found in subalgebra".to_string()
    })?;
    let d_index = sub.element_index(&d).ok_or_else(|| {
        "Element d = [1, 1] not found in subalgebra".to_string()
    })?;
...
mod tests {
    ...
    #[test]
    fn test_sd_meet_idempotent_with_z3() {
        // Test sd_meet_idempotent with z3.ua - should find witness [1, 2]
        ...
        let result = sd_meet_idempotent(&alg).expect("sd_meet_idempotent should not error");
        assert!(result.is_some(), "sd_meet_idempotent should find witness for z3");
        if let Some(witness) = result {
            assert_eq!(witness, vec![1, 2], "Witness should be [1, 2] for z3");
        }
    }
