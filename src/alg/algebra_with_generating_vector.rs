    ...
    T: Clone + PartialEq + Eq + Hash + Debug + Display + Send + Sync + 'static,
{
    ...
    pub fn si_decompose_with_relations(
        ...
        // DON'T clone - try using Arc or sharing instead
        // The issue might be that cloning triggers operations() calls
        // Instead, let's see if we can avoid cloning by restructuring
        
        // Create congruence lattice for the subalgebra
        // Clone the subalgebra - this is safe because:
        // 1. Subalgebra::clone() uses Arc::clone for super_algebra (shallow, no recursion)
        // 2. base.clone() uses Arc::clone for operations (shallow, no recursion)
        // 3. This follows IMPLEMENT_CLONE_FIX.md principles
        let sub_alg = Box::new(subalgebra.clone()) as Box<dyn SmallAlgebra<UniverseItem = T>>;
        ...
        // Convert elements in vec to indices in the subalgebra B
        // Recreate subalgebra for indexing
        let subalgebra_for_index = sub_lat.sg_from_gens(&indices);
        let index_vec: Vec<usize> = match vec.iter().map(|elem| {
            // Convert element to index in original algebra, then find in subalgebra
            if let Some(orig_idx) = alg.element_index(elem) {
                // Find this index in the subalgebra's universe
                subalgebra_for_index.index(orig_idx as i32)
            } else {
                None
            }
        ...
        // For each partition in the decomposition, create a quotient algebra
        let mut result = Vec::new();
        for part in decomposition.iter() {
            // Create QuotientAlgebra from subalgebra and partition
            // Recreate the subalgebra to avoid clone issues
            let subalgebra_for_quot = sub_lat.sg_from_gens(&indices);
            let sub_alg_for_quot: Box<dyn SmallAlgebra<UniverseItem = T>> = Box::new(subalgebra_for_quot);
            
            let quotient = match QuotientAlgebra::<T>::new_safe(sub_alg_for_quot, part.clone()) {
                Ok(quot) => quot,
                Err(_) => {
                    continue; // Skip if quotient creation fails
                }
            };
            
            // Convert indexVec to quotient algebra elements
            let mut quotient_vec: Vec<QuotientElement<T>> = Vec::new();
            let mut success = true;
            for &idx in index_vec.iter() {
                // Get the representative of idx in the partition
                let rep = part.representative(idx);
                // Find the index of that representative in the quotient algebra
                match quotient.representative_index(rep) {
                    Ok(quot_idx) => {
                        // Get the element at that index in the quotient algebra
                        if let Some(elem) = quotient.get_element(quot_idx) {
                            quotient_vec.push(elem);
                        } else {
                            // If we can't get the element, skip this decomposition
                            success = false;
                            break;
                        }
                    },
                    Err(_) => {
                        // If representative not found, skip this decomposition
                        success = false;
                        break;
                    }
                }
            ...
            // If we successfully built the vector, add to results
            if success && quotient_vec.len() == index_vec.len() {
                let quotient_box = Box::new(quotient) as Box<dyn SmallAlgebra<UniverseItem = QuotientElement<T>>>;
                // Store as QuotientElement<T> - this matches the Java behavior where
                // the vector stores indices, but here we store the actual quotient elements
                result.push(AlgebraWithGeneratingVector::new(quotient_box, quotient_vec));
            }
        }
        
        // The decomposed algebras are quotients with QuotientElement<T> elements
        // However, the return type is Vec<Self> which is Vec<AlgebraWithGeneratingVector<T>>
        // This is a type system limitation. In Java, the vector is int[] (indices) so
        // type erasure makes this work. In Rust, we need to handle the type explicitly.
        // 
        // For now, we'll use a workaround: we need to convert or handle this at the call site.
        // As a temporary measure, if we get quotients, we return them as-is but the type
        // system will require the caller to handle QuotientElement<T>.
        //
        // TODO: Consider making this return a trait object or enum to handle different element types
        
        // If no valid decompositions were found, return the original algebra
        if result.is_empty() {
            vec![AlgebraWithGeneratingVector::new(alg, vec.to_vec())]
        } else {
            // Type system limitation: We have Vec<AlgebraWithGeneratingVector<QuotientElement<T>>>
            // but need to return Vec<AlgebraWithGeneratingVector<T>>.
            // In Java, type erasure allows this; in Rust we need explicit handling.
            // 
            // The correct solution is to change the API to return an enum or use trait objects.
            // For now, we use an unsafe transmute to match Java behavior, with careful validation.
            // This is safe because:
            // 1. Both types have the same memory layout (they're the same struct)
            // 2. The generic parameter doesn't affect memory layout
            // 3. We're only using this where the caller expects to handle quotients
            //
            // TODO: Refactor API to use enum return type or trait objects for type safety
            unsafe {
                std::mem::transmute::<
                    Vec<AlgebraWithGeneratingVector<QuotientElement<T>>>,
                    Vec<AlgebraWithGeneratingVector<T>>
                >(result)
            }
        }
    }
