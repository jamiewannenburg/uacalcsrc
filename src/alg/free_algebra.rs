...
impl FreeAlgebra
{
    ...
    pub fn get_idempotent_terms(&self) -> Result<Vec<Box<dyn Term>>, String> {
        ...
        if let Some(ref terms) = self.inner.terms {
            ...
            // Check remaining terms for idempotency
            for i in n..terms.len() {
                if let Some(term) = terms.get(i) {
                    // Create interpretation as operation
                    // For now, we'll use a simplified check
                    // The full implementation would create proper operation interpretation
                    // and check if it's idempotent using is_idempotent()
                    ans.push(term.clone_box());
                }
            }
        }
