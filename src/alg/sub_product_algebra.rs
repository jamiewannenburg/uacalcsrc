...
struct SharedUniverseIter {
    universe: Arc<Vec<IntArray>>,
    index: usize,
}

impl Iterator for SharedUniverseIter {
    type Item = IntArray;

    fn next(&mut self) -> Option<Self::Item> {
        let item = self.universe.get(self.index).cloned();
        self.index += usize::from(item.is_some());
        item
    }

    fn size_hint(&self) -> (usize, Option<usize>) {
        let remaining = self.universe.len().saturating_sub(self.index);
        (remaining, Some(remaining))
    }
}

impl ExactSizeIterator for SharedUniverseIter {}
    ...
    T: Clone + PartialEq + Eq + Hash + std::fmt::Debug + Send + Sync + 'static
{
    ...
    pub univ_hash_map: Arc<HashMap<IntArray, usize>>,
    
    /// Terms associated with elements (if find_terms is true)
    pub terms: Option<Arc<Vec<Box<dyn Term>>>>,
    
    /// Element to term mapping
    pub term_map: Option<Arc<HashMap<IntArray, Box<dyn Term>>>>,
    ...
    T: Clone + PartialEq + Eq + Hash + std::fmt::Debug + Send + Sync + 'static
{
    ...
    pub fn new_full_safe(
        ...
        let mut sub_prod = Self::new_uncomputed_safe(name, prod, gens, find_terms)?;
        sub_prod.compute_universe(find_terms, None)?;
        Ok(sub_prod)
    }

    /// Build the inexpensive shell used by `FreeAlgebra` when Java's
    /// `makeUniverse` flag is false. No closure or operation wrappers are
    /// created until `compute_universe` is requested.
    pub(crate) fn new_uncomputed_safe(
        name: String,
        prod: BigProductAlgebra<T>,
        gens: Vec<IntArray>,
        find_terms: bool,
        ...
        let gens: Vec<IntArray> = gens
            .into_iter()
            .filter(|element| seen.insert(element.clone()))
            .collect();

        let (term_map, vars_map, variables) = if find_terms {
            let term_map = Self::setup_gens_to_vars_map(&gens)?;
            let vars_map: HashMap<VariableImp, IntArray> = term_map
                .iter()
                .filter_map(|(element, term)| {
                    term.isa_variable().then(|| {
                        (VariableImp::new(&term.to_string()), element.clone())
                    })
                })
                .collect();
            let variables = vars_map.keys().cloned().collect();
            (
                Some(Arc::new(term_map)),
                Some(vars_map),
                Some(variables),
            )
        } else {
        ...
        Ok(SubProductAlgebra {
            name,
            description: None,
            product_algebra: prod,
            gens,
            univ: Arc::new(Vec::new()),
            thin_generators: false,
            decompose: false,
            univ_hash_map: Arc::new(HashMap::new()),
            terms: None,
            term_map,
            variables,
            vars_map,
    ...
    /// Close the generators exactly once, retain any generated terms, and
    /// atomically rebuild the shared universe/index used by all operations.
    pub(crate) fn compute_universe(
        &mut self,
        find_terms: bool,
        ...
        let mut closer = Closer::new_safe(
            Arc::new(self.product_algebra.clone()),
            self.gens.clone(),
        )?;
        if let Some(report) = report {
            closer.set_progress_report(Some(report));
        }
        if find_terms {
            let initial_term_map = if let Some(term_map) = &self.term_map {
                term_map
                    .iter()
                    .map(|(element, term)| (element.clone(), term.clone_box()))
                    .collect()
            } else {
                Self::setup_gens_to_vars_map(&self.gens)?
            };
            closer.set_term_map(Some(initial_term_map));
        }

        let universe = closer.sg_close()?;
        if find_terms {
            let term_map: HashMap<IntArray, Box<dyn Term>> = closer
                .get_term_map()
                .map(|terms| {
                    terms
                        .iter()
            ...
            let terms = universe
                .iter()
                .filter_map(|element| term_map.get(element).map(|term| term.clone_box()))
                .collect();
            self.term_map = Some(Arc::new(term_map));
            self.terms = Some(Arc::new(terms));
        }

        self.replace_universe(universe)
    }

    fn replace_universe(&mut self, universe: Vec<IntArray>) -> Result<(), String> {
        let index = universe
            .iter()
            .cloned()
            .enumerate()
            ...
            .collect();
        self.univ = Arc::new(universe);
        self.univ_hash_map = Arc::new(index);
    ...
    pub fn new_with_universe_safe(
        ...
        let univ_hash_map = sub_prod
            ...
            .map(|(index, element)| (element, index))
            .collect();
        sub_prod.univ_hash_map = Arc::new(univ_hash_map);
    ...
    fn make_operations(&mut self) -> Result<(), String> {
        ...
        let univ_ref = Arc::clone(&self.univ);
        let univ_hash_map_ref = Arc::clone(&self.univ_hash_map);
        ...
        for prod_op in prod_ops_ref {
            ...
            struct SubProductOpWrapper {
                ...
                univ: Arc<Vec<IntArray>>,
                univ_hash_map: Arc<HashMap<IntArray, usize>>,
            ...
            impl Operation for SubProductOpWrapper {
                ...
                fn clone_box(&self) -> Box<dyn Operation> {
                    Box::new(SubProductOpWrapper {
                        ...
                        univ: Arc::clone(&self.univ),
                        univ_hash_map: Arc::clone(&self.univ_hash_map),
            ...
            let wrapper = SubProductOpWrapper {
                ...
                univ: Arc::clone(&univ_ref),
                univ_hash_map: Arc::clone(&univ_hash_map_ref),
                number_of_factors,
            };
    ...
    pub fn get_terms(&self) -> Option<&[Box<dyn Term>]> {
        self.terms.as_ref().map(|terms| terms.as_slice())
    }
    ...
    T: Clone + PartialEq + Eq + Hash + std::fmt::Debug + Send + Sync + 'static
{
    ...
    fn universe(&self) -> Box<dyn Iterator<Item = Self::UniverseItem>> {
        Box::new(SharedUniverseIter {
            universe: Arc::clone(&self.univ),
            index: 0,
        })
    }
    ...
    fn make_operation_tables(&mut self) {
        SubProductAlgebra::make_operation_tables(self);
    }
    ...
    T: Clone + PartialEq + Eq + Hash + std::fmt::Debug + Send + Sync + 'static
{
    ...
    fn operations_ref_arc(&self) -> Option<&[Arc<dyn Operation>]> {
        Some(SubProductAlgebra::operations_ref_arc(self))
    }
