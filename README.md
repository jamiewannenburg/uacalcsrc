...
This repository contains a **Rust/Python implementation** of the Universal Algebra Calculator (UACalc), intending to provide a high-performance computational algebra capabilities with Python bindings.

It was generated by Cursor, using command line wrappers to the original java code as ground truth.

There might be some shortcuts that taken by the AI's so use with caution at this stage.

Currently the java code is mirrored in rust and bindings are exposed to python via the `uacalc_lib` module. This is meant to be used similar to the old jython, but leaves the `uacalc` open for python wrappers to uacalc_lib with better and more pythonic syntax. This is not yet implemented.
