...
use crate::utils::{horner_encode, horner_decode, horner_table_size, validate_operation_args};
...
impl FlatOperationTable {
    ...
    pub fn get_value(&self, args: &[usize]) -> UACalcResult<usize> {
        ...
        let index = horner_encode(args, self.set_size)
            .ok_or_else(|| UACalcError::InvalidOperation {
...
impl TableOperation {
    ...
    pub fn new(symbol: OperationSymbol, table: Vec<Vec<usize>>, set_size: usize) -> UACalcResult<Self> {
    ...
    pub fn constant(name: String, value: usize, set_size: usize) -> UACalcResult<Self> {
        ...
        if value >= set_size { return Err(UACalcError::IndexOutOfBounds { index: value, size: set_size }); }
        if let Some(ref mut ft) = operation.table { ft.set(0, value)?; }
        ...
        F: Fn(&[usize]) -> UACalcResult<usize>,
    {
        ...
        if let Some(ref mut flat_table) = operation.table {
            ...
            let table_len = horner_table_size(arity, set_size).ok_or(UACalcError::InvalidOperation {
                message: "Table size would overflow".to_string(),
            })?;
            for idx in 0..table_len {
                let args = horner_decode(idx, arity, set_size);
                let result = f(&args)?;
                if result >= set_size { return Err(UACalcError::IndexOutOfBounds { index: result, size: set_size }); }
