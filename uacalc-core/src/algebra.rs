...
pub trait SmallAlgebra: Algebra + Send + Sync {
    ...
    fn congruence_lattice(&self) -> UACalcResult<Box<dyn crate::conlat::CongruenceLattice>> {
        use crate::conlat::lattice::BasicCongruenceLattice;
        // For now, create a basic congruence lattice
        // In a full implementation, this would need to be handled differently
        // since we can't move self into a Box
        Err(UACalcError::UnsupportedOperation {
            operation: "congruence_lattice not implemented for trait objects".to_string(),
        })
    }
}
...
impl SmallAlgebra for BasicAlgebra {
    ...
    fn congruence_lattice(&self) -> UACalcResult<Box<dyn crate::conlat::CongruenceLattice>> {
        use crate::conlat::lattice::CongruenceLatticeBuilder;
        // Create a boxed clone of self for the congruence lattice
        let algebra_box = Box::new(self.clone());
        let lattice = CongruenceLatticeBuilder::new()
            .algebra(algebra_box)
            .build()?;
        Ok(Box::new(lattice))
    }
