...
use crate::utils::{power_checked, MemoryPool, MAX_OPERATION_ARITY};
...
pub struct CongruenceGenerator<'a> {
    ...
    // Memory pool for tuple generation (arity > 3)
    tuple_pool: Option<MemoryPool<Vec<usize>>>,
}
...
impl<'a> CongruenceGenerator<'a> {
    ...
    /// Enable memory pooling for tuple generation (arity > 3)
    pub fn with_memory_pool(mut self, pool_size: usize) -> Self {
        self.tuple_pool = Some(MemoryPool::new(pool_size));
        self
    }
    ...
    pub fn compute_congruence(&mut self, pairs: &[(usize, usize)]) -> UACalcResult<BasicPartition> {
        ...
        while !self.dirty_pairs.is_empty() {
            ...
            if let Some(ref callback) = self.progress_callback {
                ...
                let denom = (size as f64) * (size as f64);
                let progress = 1.0 - (self.dirty_pairs.len() as f64 / denom.max(1.0));
    ...
    fn propagate_operation(
        ...
        // Ensure args buffer has correct size and is properly initialized
        self.args_buffer.clear();
        for _ in 0..arity {
            self.args_buffer.push(0);
        }
        ...
        for pos in 0..arity {
            ...
            for _ in 0..total_other_tuples {
                ...
                let result_a = if let Some(table) = table_opt {
                    table.value_at(&self.args_buffer[..arity])?
                } else {
                    operation.value(&self.args_buffer[..arity])?
                };
...
/// Compute the principal congruence θ(a, b) generated by a single pair
    ...
    for block in partition.blocks()? {
        for i in 0..block.len() {
            for j in (i + 1)..block.len() {
                ...
                let total_tuples =
                    power_checked(size, arity).ok_or_else(|| UACalcError::ArithmeticOverflow {
