...
use uacalc_core::term::VariableAssignment;
...
fn main() {
    ...
    for (i, assignment) in assignments.iter().enumerate() {
        ...
        println!(
            "  Assignment {}: {:?} -> {} evaluations in {:?} ({:.2} μs/iter)",
            i + 1,
            assignment,
            num_iterations,
            duration,
        ...
        println!(
            "    Memory stats: cache_size={}, stack_size={}, max_arity={}",
    ...
    println!(
        "  Average time per evaluation: {:.2} μs",
        total_time as f64 / (assignments.len() * num_iterations) as f64
    );
