...
use uacalc::util::virtuallist::{
    LongList, IntTuples, IntTuplesWithMin, FixedSizedSubsets, Subsets, Permutations, LongListUtils, TupleWithMin
};
use std::sync::Arc;
...
impl PyTupleWithMin {
    ...
    fn __str__(&self) -> String {
        format!("TupleWithMin(array_len={}, base={}, min={}, size={})", 
                self.inner.array_len, self.inner.size, self.inner.min, self.inner.size())
    }
    
    /// Python repr representation
    fn __repr__(&self) -> String {
        format!("TupleWithMin({}, {}, {})", self.inner.array_len, self.inner.size, self.inner.min)
    }
}
