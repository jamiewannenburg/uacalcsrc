...
impl PyCentralityData {
    ...
    #[new]
    fn new(left: &PyBasicBinaryRelation, right: &PyBasicBinaryRelation, delta: &PyPartition) -> PyResult<Self> {
...
impl PyCongruenceLattice {
    ...
    fn find_maximal_chain(&mut self) -> Vec<PyPartition> {
        self.inner.find_maximal_chain().into_iter().map(|p| PyPartition { inner: p }).collect()
    }
...
pub fn register_alg_module(_py: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    module_dict.del_item("PyCentralityData")?;
    ...
    module_dict.del_item("PyTypeFinder")?;
    ...
    module_dict.del_item("PyFreeAlgebra")?;
    module_dict.del_item("PyUnaryTermsMonoid")?;
