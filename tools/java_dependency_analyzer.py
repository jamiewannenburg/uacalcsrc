...
class JavaDependencyAnalyzer:
    ...
    def get_files_by_dependency_count(self, include_packages: bool = True, include_classes: bool = True) -> List[Tuple[str, int, str]]:
        """
        Return a list of files/packages/classes ordered from least dependencies to most dependencies.
        ...
        Returns:
            List of tuples: (name, outgoing_dependency_count, type) where type is 'package' or 'class'
            Ordered from least outgoing dependencies (leaves) to most outgoing dependencies (roots)
        """
        results = []
        
        if include_packages:
            # Analyze package dependencies - count outgoing dependencies only
            for package in self.packages:
                # Count outgoing dependencies (what this package depends on)
                outgoing_deps = len(self.package_dependencies.get(package, set()))
                results.append((package, outgoing_deps, 'package'))
        
        if include_classes:
            # Analyze class dependencies - count outgoing dependencies only
            for class_name in self.classes:
                # Count outgoing dependencies (what this class depends on)
                outgoing_deps = len(self.dependencies.get(class_name, set()))
                results.append((class_name, outgoing_deps, 'class'))
        
        # Sort by outgoing dependency count (ascending - leaves first)
    ...
    def get_dependency_hierarchy(self) -> Dict[str, List[str]]:
        ...
        Returns:
            ...
            Level 0 = leaves (no outgoing dependencies), higher levels = more outgoing dependencies.
        """
        hierarchy = {}
        ...
        # Group by outgoing dependency count
    ...
    def get_leaf_packages(self) -> List[str]:
        """Get packages that have no outgoing dependencies (depend on nothing else)."""
        leaves = []
        for package in self.packages:
    ...
    def get_root_packages(self) -> List[str]:
        """Get packages that have the most outgoing dependencies (depend on many things)."""
        package_dep_counts = []
        for package in self.packages:
            dep_count = len(self.package_dependencies.get(package, set()))
            package_dep_counts.append((package, dep_count))
        
        # Sort by dependency count (descending) and return top packages
        package_dep_counts.sort(key=lambda x: x[1], reverse=True)
        return [pkg for pkg, _ in package_dep_counts[:10]]  # Top 10 most dependent packages
    
    def get_leaf_classes(self) -> List[str]:
        """Get classes that have no outgoing dependencies (depend on nothing else)."""
        leaves = []
        for class_name in self.classes:
    ...
    def get_root_classes(self) -> List[str]:
        """Get classes that have the most outgoing dependencies (depend on many things)."""
        class_dep_counts = []
        for class_name in self.classes:
            dep_count = len(self.dependencies.get(class_name, set()))
            class_dep_counts.append((class_name, dep_count))
        
        # Sort by dependency count (descending) and return top classes
        class_dep_counts.sort(key=lambda x: x[1], reverse=True)
        return [cls for cls, _ in class_dep_counts[:10]]  # Top 10 most dependent classes
    ...
    def generate_text_summary(self, output_file: str):
        ...
        with open(output_file, 'w') as f:
            ...
            f.write("Files ordered by outgoing dependency count (leaves to roots):\n")
            f.write("(Format: name, outgoing_dependency_count, type)\n")
            f.write("Leaves = depend on nothing else, Roots = depend on many things\n\n")
            ...
            f.write("\nLeaf Analysis (depend on nothing else):\n")
            leaf_packages = self.get_leaf_packages()
            leaf_classes = self.get_leaf_classes()
            ...
            f.write("\nRoot Analysis (depend on many things):\n")
            root_packages = self.get_root_packages()
            root_classes = self.get_root_classes()
            f.write(f"  Root Packages (top 10): {', '.join(root_packages)}\n")
            f.write(f"  Root Classes (top 10): {', '.join(root_classes)}\n")
...
def main():
    ...
    print("="*60)
    print("Leaves = depend on nothing else, Roots = depend on many things")
    files_by_deps = analyzer.get_files_by_dependency_count()
    print("\nTop 20 files with least outgoing dependencies (leaves):")
    for i, (name, count, item_type) in enumerate(files_by_deps[:20]):
        print(f"{i+1:2d}. {name} ({count} outgoing deps, {item_type})")
    
    print(f"\nBottom 10 files with most outgoing dependencies (roots):")
    for i, (name, count, item_type) in enumerate(files_by_deps[-10:]):
        print(f"{len(files_by_deps)-9+i:2d}. {name} ({count} outgoing deps, {item_type})")
    ...
    print(f"\nRoot Analysis (depend on many things):")
    root_packages = analyzer.get_root_packages()
    root_classes = analyzer.get_root_classes()
    print(f"  Root Packages (top 10): {', '.join(root_packages[:5])}")
    if len(root_packages) > 5:
        print(f"    ... and {len(root_packages) - 5} more")
    print(f"  Root Classes (top 10): {', '.join(root_classes[:5])}")
    if len(root_classes) > 5:
        print(f"    ... and {len(root_classes) - 5} more")
