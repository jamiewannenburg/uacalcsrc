...
#[test]
fn test_basic_algebra_construction() {
    // Test creation with contiguous integer universe
    let universe = vec![0, 1, 2, 3];
    let algebra = BasicAlgebra::new("test_algebra".to_string(), universe).unwrap();
...
#[test]
fn test_algebra_operation_management() {
    let universe = vec![0, 1, 2];
    let mut algebra = BasicAlgebra::new("test".to_string(), universe).unwrap();
    
    // Create operations
    let add_op = Arc::new(TableOperation::binary("add".to_string(), 3, |a, b| (a + b) % 3));
    let neg_op = Arc::new(TableOperation::unary("neg".to_string(), 3, |x| (3 - x) % 3));
    
    // Add operations
    algebra.add_operation("add".to_string(), add_op).unwrap();
    algebra.add_operation("neg".to_string(), neg_op).unwrap();
    
    assert_eq!(algebra.operations().len(), 2);
    assert_eq!(algebra.max_arity(), 2);
    
    // Test operation retrieval
    let add_operation = algebra.operation(0).unwrap();
    assert_eq!(add_operation.arity(), 2);
    assert_eq!(add_operation.symbol().name, "add");
    
    let neg_operation = algebra.operation_by_symbol("neg").unwrap();
    assert_eq!(neg_operation.arity(), 1);
    assert_eq!(neg_operation.symbol().name, "neg");
    
    // Test operation validation
    let invalid_op = Arc::new(TableOperation::binary("invalid".to_string(), 5, |a, b| a + b));
    assert!(algebra.add_operation("invalid".to_string(), invalid_op).is_err());
}

#[test]
fn test_algebra_operation_tables() {
    let universe = vec![0, 1, 2];
    let mut algebra = BasicAlgebra::new("test".to_string(), universe).unwrap();
    ...
    algebra.make_operation_tables().unwrap();
    assert!(algebra.tables_built());
    
    // Test integer-optimized operation evaluation
    assert_eq!(algebra.operation_int_value(0, &[1, 2]).unwrap(), 0);
    assert_eq!(algebra.operation_int_value(0, &[2, 2]).unwrap(), 1);
}

#[test]
fn test_algebra_properties() {
    let universe = vec![0, 1, 2];
    let mut algebra = BasicAlgebra::new("test".to_string(), universe).unwrap();
    
    // Add idempotent operation
    let id_op = Arc::new(TableOperation::unary("id".to_string(), 3, |x| x));
    algebra.add_operation("id".to_string(), id_op).unwrap();
    ...
    let mul_op = Arc::new(TableOperation::binary("mul".to_string(), 3, |a, b| (a * b) % 3));
    algebra.add_operation("mul".to_string(), mul_op).unwrap();
    
    // Test algebra properties
    assert!(algebra.is_idempotent().unwrap());
    assert!(algebra.is_associative().unwrap());
    assert!(algebra.is_commutative().unwrap());
    
    // Test individual operation properties
    assert!(algebra.is_idempotent(0).unwrap());
    assert!(algebra.is_associative(1).unwrap());
    assert!(algebra.is_commutative(2).unwrap());
}

#[test]
fn test_algebra_subalgebra() {
    let universe = vec![0, 1, 2, 3];
    let mut algebra = BasicAlgebra::new("test".to_string(), universe).unwrap();
    ...
    algebra.add_operation("op".to_string(), op).unwrap();
...
#[test]
fn test_algebra_universe_operations() {
    let universe = vec![0, 1, 2, 3];
    let algebra = BasicAlgebra::new("test".to_string(), universe).unwrap();
    
    // Test element to index conversion
    assert_eq!(algebra.element_to_index(0).unwrap(), 0);
    assert_eq!(algebra.element_to_index(2).unwrap(), 2);
    assert!(algebra.element_to_index(4).is_err());
    
    // Test index to element conversion
    assert_eq!(algebra.index_to_element(0).unwrap(), 0);
    assert_eq!(algebra.index_to_element(2).unwrap(), 2);
...
#[test]
fn test_algebra_from_operations() {
    let universe = vec![0, 1, 2];
    let add_op = Arc::new(TableOperation::binary("add".to_string(), 3, |a, b| (a + b) % 3));
    let neg_op = Arc::new(TableOperation::unary("neg".to_string(), 3, |x| (3 - x) % 3));
    
    let operations = vec![
        ("add".to_string(), add_op),
        ("neg".to_string(), neg_op),
    ];
    
    let algebra = BasicAlgebra::from_operations("test".to_string(), universe, operations).unwrap();
    ...
    let add_operation = algebra.operation_by_symbol("add").unwrap();
    assert_eq!(add_operation.arity(), 2);
...
fn test_algebra_operation_arc() {
    ...
    // Test operation_arc
    let op_arc = algebra.operation_arc(0).unwrap();
    assert_eq!(op_arc.arity(), 2);
    
    // Test operation_arc_by_symbol
    let op_arc_symbol = algebra.operation_arc_by_symbol("add").unwrap();
...
#[test]
fn test_algebra_operation_direct() {
    let universe = vec![0, 1, 2];
    let mut algebra = BasicAlgebra::new("test".to_string(), universe).unwrap();
    ...
    let op = algebra.operation_direct(0).unwrap();
...
#[test]
fn test_algebra_total_operations() {
    let universe = vec![0, 1, 2];
    let mut algebra = BasicAlgebra::new("test".to_string(), universe).unwrap();
...
fn test_algebra_serialization() {
    ...
    // Test serialization
    let serialized = serde_json::to_string(&algebra).unwrap();
    let deserialized: BasicAlgebra = serde_json::from_str(&serialized).unwrap();
    
    // Verify deserialized algebra works correctly
    assert_eq!(deserialized.cardinality(), 3);
    assert_eq!(deserialized.operations().len(), 1);
    assert_eq!(deserialized.operation(0).unwrap().arity(), 2);
}
...
#[test]
fn test_algebra_edge_cases() {
    // Test empty universe (should not be allowed)
    assert!(BasicAlgebra::new("empty".to_string(), vec![]).is_err());
    
    // Test single element universe
    let universe = vec![0];
    let algebra = BasicAlgebra::new("single".to_string(), universe).unwrap();
    assert_eq!(algebra.cardinality(), 1);
    assert!(algebra.is_finite());
    
    // Test large universe
    let large_universe: Vec<usize> = (0..1000).collect();
    let algebra = BasicAlgebra::new("large".to_string(), large_universe).unwrap();
    assert_eq!(algebra.cardinality(), 1000);
    assert!(algebra.is_finite());
}

#[test]
fn test_algebra_complex_scenarios() {
    let universe = vec![0, 1, 2, 3, 4];
    let mut algebra = BasicAlgebra::new("complex".to_string(), universe).unwrap();
    
    // Add multiple operations with different arities
    let const_op = Arc::new(TableOperation::constant("zero".to_string(), 0, 5).unwrap());
    let neg_op = Arc::new(TableOperation::unary("neg".to_string(), 5, |x| (5 - x) % 5));
    let add_op = Arc::new(TableOperation::binary("add".to_string(), 5, |a, b| (a + b) % 5));
    ...
    algebra.add_operation("zero".to_string(), const_op).unwrap();
    algebra.add_operation("neg".to_string(), neg_op).unwrap();
    algebra.add_operation("add".to_string(), add_op).unwrap();
    algebra.add_operation("ternary".to_string(), ternary_op).unwrap();
    ...
    assert_eq!(algebra.operation_int_value(0, &[]).unwrap(), 0); // Constant
    assert_eq!(algebra.operation_int_value(1, &[2]).unwrap(), 3); // Negation
    assert_eq!(algebra.operation_int_value(2, &[3, 4]).unwrap(), 2); // Addition
    assert_eq!(algebra.operation_int_value(3, &[1, 2, 3]).unwrap(), 1); // Ternary
...
#[test]
fn test_algebra_performance() {
    let universe: Vec<usize> = (0..100).collect();
    let mut algebra = BasicAlgebra::new("performance".to_string(), universe).unwrap();
    
    // Add a binary operation
    let add_op = Arc::new(TableOperation::binary("add".to_string(), 100, |a, b| (a + b) % 100));
    algebra.add_operation("add".to_string(), add_op).unwrap();
    ...
    for _ in 0..iterations {
        for a in 0..10 {
            for b in 0..10 {
                algebra.operation_int_value(0, &[a, b]).unwrap();
            }
        }
    }
...
#[test]
fn test_algebra_java_compatibility() {
    let universe = vec![0, 1, 2];
    let mut algebra = BasicAlgebra::new("java_compat".to_string(), universe).unwrap();
    
    let add_op = Arc::new(TableOperation::binary("add".to_string(), 3, |a, b| (a + b) % 3));
    algebra.add_operation("add".to_string(), add_op).unwrap();
    
    // Test Java-compatible method names
    assert_eq!(algebra.element_to_index(1).unwrap(), 1);
    assert_eq!(algebra.index_to_element(1).unwrap(), 1);
    assert_eq!(algebra.operation_int_value(0, &[1, 2]).unwrap(), 0);
