...
fn test_partition_normalization() {
    ...
    assert_eq!(partition.representative(0).unwrap(), partition.representative(1).unwrap());
    assert_eq!(partition.representative(2).unwrap(), partition.representative(3).unwrap());
}
...
fn test_partition_edge_cases() {
    ...
    assert!(BasicPartition::new(0).is_err());
    
    // Test single element partition
    let partition = BasicPartition::new(1).unwrap();
    assert_eq!(partition.num_blocks(), 1);
    assert!(partition.is_zero());
    assert!(partition.is_one());
    assert!(partition.is_uniform());
...
fn test_partition_error_handling() {
    let partition = BasicPartition::new(5).unwrap();
