...
fn test_taylor_spec_creation() {
    ...
    let equations = vec![
        (IntArray::from_vec(vec![0, 0, 0, 0]), IntArray::from_vec(vec![0, 0, 0, 0])),
        (IntArray::from_vec(vec![1, 1, 1, 1]), IntArray::from_vec(vec![1, 1, 1, 1])),
    ];
...
fn test_taylor_equation_satisfaction() {
    ...
    assert!(spec.satisfies_equations(&const_0));
    assert!(spec.satisfies_equations(&const_1));
}
...
fn test_taylor_canonical_forms() {
    ...
    let equations = vec![
        (IntArray::from_vec(vec![0, 1, 0, 1]), IntArray::from_vec(vec![0, 1, 0, 1])),
    ];
    ...
    let canonical = canonical_form(&input, &spec.root_map);
