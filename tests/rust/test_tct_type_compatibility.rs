...
fn test_tct_type_small_boolean_algebra() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let meet_op = Arc::new(TableOperation::binary("meet".to_string(), 2, |a, b| {
        if a == 0 || b == 0 { 0 } else { 1 }
    })?);
    
    // Add join operation (max)
    let join_op = Arc::new(TableOperation::binary("join".to_string(), 2, |a, b| {
        if a == 1 || b == 1 { 1 } else { 0 }
    })?);
...
fn test_tct_type_small_group_algebra() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let inv_op = Arc::new(TableOperation::unary("inv".to_string(), 3, |x| {
        (3 - x) % 3
    })?);
...
fn test_tct_type_small_lattice_algebra() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Add meet operation
    let meet_op = Arc::new(TableOperation::binary("meet".to_string(), 3, |a, b| {
        match (a, b) {
            (0, _) | (_, 0) => 0,
            (1, 1) => 1,
            (1, 2) | (2, 1) => 1,
    ...
    // Add join operation
    let join_op = Arc::new(TableOperation::binary("join".to_string(), 3, |a, b| {
        match (a, b) {
            (2, _) | (_, 2) => 2,
            (1, 1) => 1,
            (0, 1) | (1, 0) => 1,
...
fn test_tct_type_memory_estimation() -> Result<(), Box<dyn std::error::Error>> {
    // Test memory estimation for TCT analysis
    let universe = vec![0, 1, 2, 3, 4];
    let algebra = BasicAlgebra::new("test5".to_string(), universe)?;
    
    let analyzer = MalcevAnalyzer::new();
    let memory_estimate = analyzer.estimate_tct_memory_usage(&algebra)?;
    
    // Memory estimate should be reasonable for size 5 algebra
    assert!(memory_estimate > 0);
    assert!(memory_estimate < 1_000_000_000); // Less than 1GB
...
fn test_tct_type_consistency() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let op = Arc::new(TableOperation::binary("op".to_string(), 3, |a, b| {
        (a + b) % 3
    })?);
...
fn test_tct_type_memory_limit_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Add some operations
    let op1 = Arc::new(TableOperation::binary("op1".to_string(), 10, |a, b| {
        (a + b) % 10
    })?);
    
    let op2 = Arc::new(TableOperation::unary("op2".to_string(), 10, |x| {
        (x + 1) % 10
    })?);
