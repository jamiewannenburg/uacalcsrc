...
# Mock Horner implementation for testing compatibility
class MockHorner:
    """Mock implementation of Horner encoding/decoding for compatibility testing"""
    ...
    def horner_encode(args: List[int], base: int) -> Optional[int]:
        ...
        for arg in args:
            ...
            new_index = index * base
            if new_index < index:  # Overflow check
                return None
            
            # Check for overflow in addition
            final_index = new_index + arg
            if final_index < new_index:  # Overflow check
                return None
    ...
    def horner_table_size(arity: int, base: int) -> Optional[int]:
        ...
        for _ in range(arity):
            new_size = size * base
            if new_size < size:  # Overflow check
                return None
            size = new_size
    ...
    def horner_encode_mixed_radix(args: List[int], sizes: List[int]) -> Optional[int]:
        ...
        for i in range(k - 2, -1, -1):
            ...
            new_ans = sizes[i] * ans
            if new_ans < ans:  # Overflow check
                return None
            
            final_ans = new_ans + args[i]
            if final_ans < new_ans:  # Overflow check
                return None
